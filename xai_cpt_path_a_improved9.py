...
def scoring(model, X_feat, y_true):
    ...
    y_pred = model.predict(np.array(X_feat))
...
for lp, ld, label in loss_configs:
    ...
    model_ab = PhysicsConstrainedJointSAE(
        n_seq=N_SEQ, d_joint=JOINT_D_FEAT, latent_dim=JOINT_LATENT,
        lambda_phys=lp, lambda_depth=ld, name=f'ab_{label[:3]}_{int(lp*10)}_{int(ld*10)}')
...
shap.plots.scatter(shap_all[:, top_lat_name], color=shap_all[:, COL_GWD],
                   show=False)
plt.title(f'Dependency: {top_lat_name} (colored by {COL_GWD})')
plt.tight_layout()
...
n_examples = 3
sample_indices = [0, len(X_j_te)//2, len(X_j_te)-1]  # avoid -1: x[-1:0] is empty slice
...
for row_idx, si in enumerate(sample_indices):
    ...
    # NOTE: Keras 3 predict() has UnboundLocalError bug on batch_size=1;
    # use direct model call instead
    # Use Ic slice from joint test set (avoids index mismatch with X_ic_te)
    ic_base_r = IC_SCALE * sae_ic_base(
        X_j_te[si:si+1, :, :20].astype(np.float32), training=False).numpy()[0].flatten()
