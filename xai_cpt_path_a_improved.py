# -*- coding: utf-8 -*-
"""xai_cpt_path_a_improved_7de40f6ebdb24e4385876c8bd9148522.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/11Py5_S4xW_zyESb6lkGcOkjwcGAw9j7d
...
# Match autoencoder training data to site feature GWD via LocationID
# id_data: (N,)  matches df.LocationID

gwd_full = df.set_index('LocationID')['GWD']
...
# We need to match CPT profiles (id_data) to site features (df.LocationID)

def build_feature_matrix(lat_feats, id_cpt, df_site, basic_feats):
    """
    Merge latent features with site features using LocationID.
    ...
    lat_df['LocationID'] = id_cpt

    merged = df_site[['LocationID', 'Target'] + basic_feats].merge(
        lat_df, on='LocationID', how='inner')

    X = merged.drop(columns=['LocationID', 'Target']).values
    y = merged['Target'].values
    ids = merged['LocationID'].values
    return X, y, ids
...
shap.plots.scatter(shap_all[:, top_lat_name], color=shap_all[:, 'GWD'],
                   ax=ax, show=False)
ax.set_title(f'Dependency: {top_lat_name} (colored by GWD)')
plt.tight_layout()
