...
def build_single_sae(n_seq, d_model, latent_dim,
    ...
    x = enc_input + pe
    attn1, _ = keras.layers.MultiHeadAttention(
        num_heads=initial_heads, key_dim=d_model, dropout=dropout,
        name=f'{name_prefix}enc_at1')(x, x, return_attention_scores=True)
    x = keras.layers.Add()([enc_input, attn1])
    x = keras.layers.LayerNormalization()(x)
    attn2, _ = keras.layers.MultiHeadAttention(
        num_heads=intermediate_heads, key_dim=d_model, dropout=dropout,
        name=f'{name_prefix}enc_at2')(x, x, return_attention_scores=True)
    ...
    x = x + pe
    attn3, _ = keras.layers.MultiHeadAttention(
        num_heads=intermediate_heads, key_dim=d_model, dropout=dropout,
        name=f'{name_prefix}dec_at1')(x, x, return_attention_scores=True)
    x = keras.layers.Add()([x, attn3])
    x = keras.layers.LayerNormalization()(x)
    attn4, _ = keras.layers.MultiHeadAttention(
        num_heads=initial_heads, key_dim=d_model, dropout=dropout,
        name=f'{name_prefix}dec_at2')(x, x, return_attention_scores=True)
...
# Save
enc_ic_base.save(f'{model_path}/enc_ic_baseline.h5')
enc_qc_base.save(f'{model_path}/enc_qc_baseline.h5')
dec_ic_base.save(f'{model_path}/dec_ic_baseline.h5')
...
def build_joint_sae(n_seq, d_joint, latent_dim,
    ...
    recon_ic = keras.layers.Dense(D_FEAT, activation='sigmoid',
                                  name=f'{name_prefix}out_ic')(x)  # (N, 10, 20)
    recon_qc = keras.layers.Dense(D_FEAT, activation='sigmoid',
...
callbacks_joint = [
    ...
    keras.callbacks.ModelCheckpoint(f'{model_path}/sae_joint.h5',
...
enc_joint.save(f'{model_path}/enc_joint.h5')
dec_joint.save(f'{model_path}/dec_joint.h5')
...
# Build model to inspect
_ = phys_sae.encode(tf.zeros((2, N_SEQ, JOINT_D_FEAT)))
_ = phys_sae.decode(tf.zeros((2, JOINT_LATENT)), tf.zeros((2, 1)))
print(f"Physics SAE trainable params: {phys_sae.count_params():,}")
...
# Save weights
phys_sae.save_weights(f'{model_path}/phys_sae_weights.h5')
...
for n_seq_ab, d_feat_ab, label in reshape_configs:
    ...
    sae_ab.fit(X_ab_tr, X_ab_tr, epochs=EPOCHS_ABLATION, batch_size=BATCH_SIZE,
    ...
    val_loss = min(sae_ab.history.history['val_loss'])
...
for lat_ab in latent_configs:
    ...
    _, _, sae_lat = build_joint_sae(
        ...
        initial_heads=2, intermediate_heads=4)
    sae_lat.fit(X_j_tr, X_j_tr, epochs=EPOCHS_ABLATION, batch_size=BATCH_SIZE,
    ...
    val_loss  = min(sae_lat.history.history['val_loss'])
...
for lp, ld, label in loss_configs:
    ...
    model_ab.encode(tf.zeros((2, N_SEQ, JOINT_D_FEAT)))
    model_ab.decode(tf.zeros((2, JOINT_LATENT)), tf.zeros((2, 1)))
...
lat_all_phys, _ = phys_sae(
    (X_joint_all.astype(np.float32),
     gwd_aligned.astype(np.float32)), training=False)
lat_all_phys = lat_all_phys.numpy()  # wait—this is reconstruction, not latent

# Correct: get latent codes
