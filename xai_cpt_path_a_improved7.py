...
def scoring(model, X_feat, y_true):
    ...
    y_pred = model.predict(X_feat) if len(X_feat) > 1 else model(np.array(X_feat, dtype=np.float32), training=False).numpy().flatten()
...
for row_idx, si in enumerate(sample_indices):
    ...
    ic_base_r = IC_SCALE * sae_ic_base(
        X_ic_te[si:si+1].astype(np.float32), training=False).numpy()[0].flatten()
